...
ClassImp(CXGFileBrowser)

////////////////////////////////////////////////////////////////////////////////
/// CXGFileBrowser constructor.

CXGFileBrowser::CXGFileBrowser(const TGWindow *p, TBrowser* b, UInt_t w, UInt_t h)
    : TGMainFrame(p, w, h), TBrowserImp(b), fNewBrowser(nullptr)
{
    if (p && p != gClient->GetDefaultRoot())
        fNewBrowser = dynamic_cast<TRootBrowser*>(const_cast<TGWindow*>(p->GetMainFrame()));
    //   if (fNewBrowser)
...
void CXGFileBrowser::CreateBrowser()
{
    fCachedPic  = nullptr;
    SetCleanup(kDeepCleanup);

    fTopFrame = new TGHorizontalFrame(this, 100, 30);
    fDrawOption = new TGComboBox(fTopFrame, "hist col");
    TGTextEntry *dropt_entry = fDrawOption->GetTextEntry();
    dropt_entry->SetToolTipText("Object Draw Option: hist, same, norm, add, add(fact), div(fact), mult(fact)", 300);

    fDrawOption->Resize(80, 20);
    TGListBox *lb = fDrawOption->GetListBox();
    lb->Resize(lb->GetWidth(), 120);
    Int_t dropt = 1;
    fDrawOption->AddEntry("hist col", dropt++);
    fDrawOption->AddEntry("norm", dropt++);
    fDrawOption->AddEntry("add(1)", dropt++);
    fDrawOption->AddEntry("div(1)", dropt++);
    fDrawOption->AddEntry("mult(1)", dropt++);
    ...
    //                       kLHintsLeft, 2, 2, 2, 2));
    //   fSortButton->Connect("Clicked()", "CXGFileBrowser", this, "ToggleSort()");
    ...
    fTopFrame->AddFrame(fFilterButton, new TGLayoutHints(kLHintsCenterY |
                                                         kLHintsLeft, 2, 2, 2, 2));
    fFilterButton->Connect("Clicked()", "CXGFileBrowser", this, "RequestFilter()");
    ...
    //                       kLHintsLeft, 2, 5, 2, 2));
    //   fRefreshButton->Connect("Clicked()", "CXGFileBrowser", this, "Refresh()");
    ...
    fListTree->Connect("DoubleClicked(TGListTreeItem *, Int_t)",
                       "CXGFileBrowser", this, "DoubleClicked(TGListTreeItem *, Int_t)");
    fListTree->Connect("Clicked(TGListTreeItem *, Int_t, Int_t, Int_t)",
                       "CXGFileBrowser", this, "Clicked(TGListTreeItem *, Int_t, Int_t, Int_t)");
    fListTree->Connect("Checked(TObject*, Bool_t)", "CXGFileBrowser",
    ...
    fBotFrame->AddFrame(fFileType, new TGLayoutHints(kLHintsLeft | kLHintsTop |
                                                     kLHintsExpandX, 2, 2, 2, 2));
    fFileType->Connect("Selected(Int_t)", "CXGFileBrowser", this,
                       "ApplyFilter(Int_t)");
    fFileType->GetTextEntry()->Connect("ReturnPressed()", "CXGFileBrowser",
    ...
    TQObject::Connect("TGHtmlBrowser", "Clicked(char*)",
                      "CXGFileBrowser", this, "Selected(char*)");

    TQObject::Connect("TPad", "Modified()",
                      "CXGFileBrowser", this, "PadModified()");
...
void CXGFileBrowser::Add(TObject *obj, const char *name, Int_t check)
{
    ...
    if (obj && obj->InheritsFrom("TKey") && (obj->IsA() != TClass::Class()))
        ...
        if (check > -1) {
            ...
            if (obj->InheritsFrom("TRemoteObject"))
                ...
                while (top->GetParent()) {
                    auto *tobj = static_cast<TObject*>(top->GetUserData());
            ...
            if (isRemote) {
                auto *robj = dynamic_cast<TRemoteObject *>(obj);
...
void CXGFileBrowser::AddRemoteFile(TObject *obj)
{
    Bool_t      is_link;
    Int_t       type;
    TString     filename;
    ...
    if (R_ISDIR(type) || fFilter == nullptr ||
        ...
        auto *pic = const_cast<TGPicture*>(spic); pic->AddReference();
...
void CXGFileBrowser::BrowseObj(TObject *obj)
{
    if (fNewBrowser)
        fNewBrowser->SetActBrowser(this);
    if (obj != gROOT) {
...
void CXGFileBrowser::Checked(TObject *obj, Bool_t checked)
{
    if (fNewBrowser)
...
Option_t *CXGFileBrowser::GetDrawOption() const
{
...
void CXGFileBrowser::GetFilePictures(const TGPicture **pic, Int_t file_type,
...
void CXGFileBrowser::RecursiveRemove(TObject *obj)
{
...
void CXGFileBrowser::Refresh(Bool_t /*force*/)
{
    TTimer::SingleShot(200, "CXGFileBrowser", this, "Update()");
...
void CXGFileBrowser::Update()
{
    ...
    if (curr) {
        auto *obj = static_cast<TObject*>(curr->GetUserData());
...
void CXGFileBrowser::AddFSDirectory(const char *entry, const char *path,
                                    Option_t *opt)
{
    TGListTreeItem *item = nullptr;
    if ((opt == nullptr) || (!opt[0])) {
        if (fRootDir == nullptr && !fListTree->FindChildByName(nullptr, rootdir))
            fRootDir = fListTree->AddItem(nullptr, rootdir);
        return;
    }
    ...
    if (item && path) {
        ...
        if (infos.Contains("Removable"))
            pic = const_cast<TGPicture *>(gClient->GetPicture("fdisk_t.xpm"));
        else if (infos.Contains("Local"))
            pic = const_cast<TGPicture *>(gClient->GetPicture("hdisk_t.xpm"));
        else if (infos.Contains("CD"))
            pic = const_cast<TGPicture *>(gClient->GetPicture("cdrom_t.xpm"));
        else if (infos.Contains("Network"))
            pic = const_cast<TGPicture *>(gClient->GetPicture("netdisk_t.xpm"));
        if (pic)
...
void CXGFileBrowser::AddKey(TGListTreeItem *itm, TObject *obj, const char *name)
{
    // Int_t from, to;
...
void CXGFileBrowser::ApplyFilter(Int_t id)
{
    // Long64_t size;
    ...
    if ((id > 1) && (id < 5))
        ...
        auto *lbe = dynamic_cast<TGTextLBEntry*>(fFileType->GetSelectedEntry());
        if (lbe) {
...
void CXGFileBrowser::Chdir(TGListTreeItem *item)
{
    if (item) {
        TGListTreeItem *i = item;
        while (i) {
            auto *obj = static_cast<TObject*>(i->GetUserData());
...
void CXGFileBrowser::CheckRemote(TGListTreeItem *item)
{
    if (!item) return;
    ...
    if (obj) {
        ...
        if (item->GetParent() && item->GetParent()->GetUserData() &&
                static_cast<TObject*>(item->GetParent()->GetUserData())->InheritsFrom("TApplicationRemote")) {
            ...
            auto *topobj = static_cast<TObject*>(top->GetUserData());
...
Bool_t CXGFileBrowser::CheckFiltered(TGListTreeItem *item, Bool_t but)
{
    Bool_t found = kFALSE;
    TString filter;
...
Bool_t CXGFileBrowser::CheckSorted(TGListTreeItem *item, Bool_t /*but*/)
{
    Bool_t found = kFALSE;
    TGListTreeItem *itm;
...
void CXGFileBrowser::Clicked(TGListTreeItem *item, Int_t btn, Int_t x, Int_t y)
{
    Long64_t size = 0;
    Long_t id = 0, flags = 0, modtime = 0;
    fListLevel = item;
    ...
    if (obj && (!obj->InheritsFrom("TObjString") ||
        ...
        if (obj->InheritsFrom("TKey") && (obj->IsA() != TClass::Class())) {
            ...
            if (clname && strcmp(clname, "TGeoManager")) {
                ...
                void *add = gDirectory->FindObjectAny(name.Data());
                if (add && cl->IsTObject()) {
                    obj = static_cast<TObject*>(add);
    ...
    if (selected && gPad && IsObjectEditable(selected->IsA())) {
        ...
        if (ved) {
            auto *ged = dynamic_cast<TGedEditor*>(ved);
...
TString CXGFileBrowser::FullPathName(TGListTreeItem* item)
{
...
TString CXGFileBrowser::DirName(TGListTreeItem* item)
{
    TString dirname;
    TString fullpath = FullPathName(item);
...
static const TGPicture *MakeLinkPic(const TGPicture *pic)
{
    if (pic) {
        ...
        const TGPicture *merged = gClient->GetPicturePool()->GetPicture(lnk_name.Data(),
...
void CXGFileBrowser::DoubleClicked(TGListTreeItem *item, Int_t /*btn*/)
{
    ...
    if (obj && !obj->InheritsFrom("TSystemFile")) {
        ...
        if (obj->InheritsFrom("TDirectory") && (obj->IsA() != TClass::Class())) {
            ...
            if (!strcmp(robj->GetClassName(), "TKey")) {
                ...
                auto *probj = static_cast<TRemoteObject *>(parent->GetUserData());
        ...
        if (files) {
            ...
            while ((file=(TSystemFile*)nextf())) {
                ...
                if (!file->IsDirectory() && (fFilter == nullptr ||
                    ...
                    const TGPicture *pic = gClient->GetMimeTypeList()->GetIcon(pname, kTRUE);
                    if (!pic)
                        pic = fFileIcon;
                    if (sbuf.fIsLink)
                        pic = MakeLinkPic(pic);
        ...
        if (fname.EndsWith(".root")) {
            ...
            if (rfile) {
                ...
                if (item->GetUserData()) {
                    ...
                    auto *obj2 = static_cast<TObject *>(item->GetUserData());
                    // only delete TObjString as they are the only objects
                    // created who have to be deleted
                    auto *ostr = dynamic_cast<TObjString *>(obj2);
                    delete ostr;
                }
            ...
            if (fNewBrowser) {
                ...
                if (fe) {
                    auto *embed = dynamic_cast<TGCompositeFrame *>(fe->fFrame);
...
Long_t CXGFileBrowser::XXExecuteDefaultAction(TObject *obj)
{
    char action[512];
    TString act;
    TString ext = obj->GetName();
...
char *CXGFileBrowser::FormatFileInfo(const char *fname, Long64_t size, Long_t modtime)
{
...
void CXGFileBrowser::GetObjPicture(const TGPicture **pic, TObject *obj)
{
    ...
    *pic = gClient->GetMimeTypeList()->GetIcon(iconname, kTRUE);
    if (!(*pic) && xpm) {
        if (im && im->SetImageBuffer((char**)&name, TImage::kXpm)) {
            ...
            *pic = gClient->GetPicturePool()->GetPicture(iconname, im->GetPixmap(), im->GetMask());
        }
...
void CXGFileBrowser::GotoDir(const char *path)
{
    ...
    if (first.Length() == 2 && first.EndsWith(":")) {
        TList *curvol  = gSystem->GetVolumes("cur");
        if (curvol) {
            auto *drive = dynamic_cast<TNamed*>(curvol->At(0));
...
void CXGFileBrowser::PadModified()
{
...
void CXGFileBrowser::RequestFilter()
{
    char filter[1024];
    if (!fListLevel)
        return;
    ...
    if (((filter[0] == 0) && (filter[1] == 1)) || !strcmp(filter, "*")) {
...
void CXGFileBrowser::Selected(char *)
{
...
void CXGFileBrowser::ToggleSort()
{
